...
type RepositoryInterface interface {
	...
	GetByID(id int) (auth Auth, err error)
}
...
func (s Service) GetByID(id int) (auth Auth, err error) {
	// Gunakan repository untuk mengambil data Auth berdasarkan ID
	auth, err = s.repo.GetByID(id)
	if err != nil {
		log.Println("error when try to get auth by ID with error", err.Error())
	}
	return auth, err
}
